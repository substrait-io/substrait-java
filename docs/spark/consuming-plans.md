...
```scala
val buffer = Files.readAllBytes(Paths.get("spark_substrait.plan"))
...
!!! warning "Pass `SparkExtension.COLLECTION` for Spark-produced plans"
    ...
    `SparkExtension` is a Scala `object`; its `COLLECTION` value is `SparkExtension.COLLECTION` in
    Scala (and `SparkExtension.COLLECTION()` when called from Java).
