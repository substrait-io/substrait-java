...
import io.substrait.relation.Update;
...
public class UpdateRelRoundtripTest extends TestBase {
  ...
  void update() {
    ...
    List<Update.TransformExpression> transformations =
        Arrays.asList(
            Update.TransformExpression.builder().columnTarget(0).transformation(fnAdd(1)).build(),
            Update.TransformExpression.builder().columnTarget(1).transformation(fnAdd(2)).build());
