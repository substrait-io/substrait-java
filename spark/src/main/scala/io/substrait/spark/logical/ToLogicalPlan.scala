...
class ToLogicalPlan(spark: SparkSession = SparkSession.builder().getOrCreate())
  ...
  override def visit(write: NamedWrite): LogicalPlan = {
    val child = write.getInput.accept(this)
    val names = write.getNames
    if (names.size() < 3) {
      throw new UnsupportedOperationException(
        s"NamedWrite requires three names (catalog, database, table): $names")
    }
    val id = TableIdentifier(names.get(2), Some(names.get(1)), Some(names.get(0)))
  ...
  override def visit(write: ExtensionWrite): LogicalPlan = {
    val child = write.getInput.accept(this)
