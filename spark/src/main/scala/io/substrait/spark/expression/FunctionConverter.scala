...
class FunctionFinder[F <: SimpleExtension.Function, T](
  ...
  def attemptMatch(expression: Expression, operands: Seq[FunctionArg]): Option[T] = {
    ...
    if (operands.isEmpty) {
      ...
      validateOutputType(variant, operands, outputType)
            ...
            singularInputType
              ...
              .map(
                declaration => {
                  ...
                  validateOutputType(declaration, coercedArgs, outputType)
  ...
  private def validateOutputType(
      function: SimpleExtension.Function,
      arguments: Seq[FunctionArg],
      t: Type): Unit = {
    // Invokes the `validateOutputType` method in the core module (which at the moment doesn't do anything).
    // It's only defined for functions with arguments that are all Expression objects.
    // When it eventually gets implemented, it might be able to handle FunctionArg objects (the base interface).
    if (arguments.forall(arg => arg.isInstanceOf[SExpression])) {
      function.validateOutputType(
        JavaConverters.bufferAsJavaList(arguments.map(_.asInstanceOf[SExpression]).toBuffer),
        t)
    }
  }
}
