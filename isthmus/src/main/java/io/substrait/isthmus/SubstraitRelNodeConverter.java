...
public class SubstraitRelNodeConverter extends AbstractRelVisitor<RelNode, RuntimeException> {
  ...
  @Override
  public RelNode visit(NamedUpdate update) {
    relBuilder.scan(update.getNames());
    RexNode condition = update.getCondition().accept(expressionRexConverter);
    ...
    for (AbstractUpdate.TransformExpression transform : update.getTransformations()) {
      ...
      sourceExpressionList.add(transform.getTransformation().accept(expressionRexConverter));
    }
  ...
  @Override
  public RelNode visit(VirtualTableScan virtualTableScan) {
    ...
    for (final Expression.StructLiteral row : virtualTableScan.getRows()) {
      ...
      for (final Expression.Literal literal : row.fields()) {
        final RexNode rexNode = literal.accept(expressionRexConverter);
  ...
  @Override
  public RelNode visit(NamedWrite write) {
    RelNode input = write.getInput().accept(this);
    assert relBuilder.getRelOptSchema() != null;
    ...
    TableModify.Operation operation =
        switch (write.getOperation()) {
          case INSERT -> TableModify.Operation.INSERT;
          case DELETE -> TableModify.Operation.DELETE;
          default ->
