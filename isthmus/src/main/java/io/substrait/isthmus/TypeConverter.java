...
public class TypeConverter {
  ...
  public TypeConverter(final UserTypeMapper userTypeMapper) {
    this.userTypeMapper = userTypeMapper;
  }

  public Type toSubstrait(final RelDataType type) {
    return toSubstrait(type, new ArrayList<>());
  }

  public NamedStruct toNamedStruct(final RelDataType type) {
    if (type.getSqlTypeName() != SqlTypeName.ROW) {
      throw new IllegalArgumentException("Expected type of struct.");
    }

    final ArrayList<String> names = new ArrayList<String>();
    final Struct struct = (Type.Struct) toSubstrait(type, names);
    return NamedStruct.of(names, struct);
  }

  private Type toSubstrait(final RelDataType type, final List<String> names) {
    // Check for user mapped types first as they may re-use SqlTypeNames
    final Type userType = userTypeMapper.toSubstrait(type);
    if (userType != null) {
      return userType;
    }
    ...
    switch (type.getSqlTypeName()) {
      ...
      case MAP:
        {
          final MapSqlType map = (MapSqlType) type;
          return creator.map(
              toSubstrait(map.getKeyType(), names), toSubstrait(map.getValueType(), names));
        }
      case ROW:
        {
          final ArrayList<Type> children = new ArrayList<Type>();
  ...
  public RelDataType toCalcite(
      final RelDataTypeFactory relDataTypeFactory, final TypeExpression typeExpression) {
  ...
  private static class ToRelDataType
    ...
    @Override
    public RelDataType visit(final Type.Bool expr) {
      return t(n(expr), SqlTypeName.BOOLEAN);
    }

    @Override
    public RelDataType visit(final Type.I8 expr) {
      return t(n(expr), SqlTypeName.TINYINT);
    }

    @Override
    public RelDataType visit(final Type.I16 expr) {
      return t(n(expr), SqlTypeName.SMALLINT);
    }

    @Override
    public RelDataType visit(final Type.I32 expr) {
      return t(n(expr), SqlTypeName.INTEGER);
    }

    @Override
    public RelDataType visit(final Type.I64 expr) {
      return t(n(expr), SqlTypeName.BIGINT);
    }

    @Override
    public RelDataType visit(final Type.FP32 expr) {
      return t(n(expr), SqlTypeName.REAL);
    }

    @Override
    public RelDataType visit(final Type.FP64 expr) {
      return t(n(expr), SqlTypeName.DOUBLE);
    }

    @Override
    public RelDataType visit(final Type.Str expr) {
      return t(n(expr), SqlTypeName.VARCHAR);
    }

    @Override
    public RelDataType visit(final Type.Binary expr) {
      return t(n(expr), SqlTypeName.VARBINARY);
    }

    @Override
    public RelDataType visit(final Type.Date expr) {
      return t(n(expr), SqlTypeName.DATE);
    }

    @Override
    public RelDataType visit(final Type.Time expr) {
      return t(n(expr), SqlTypeName.TIME, 6);
    }

    @Override
    public RelDataType visit(final Type.TimestampTZ expr) {
      return t(n(expr), SqlTypeName.TIMESTAMP_WITH_LOCAL_TIME_ZONE, 6);
    }

    @Override
    public RelDataType visit(final Type.Timestamp expr) {
      return t(n(expr), SqlTypeName.TIMESTAMP, 6);
    }

    @Override
    public RelDataType visit(final Type.PrecisionTime expr) {
      final int maxPrecision = typeFactory.getTypeSystem().getMaxPrecision(SqlTypeName.TIME);
    ...
    @Override
    public RelDataType visit(final Type.PrecisionTimestamp expr) {
      final int maxPrecision = typeFactory.getTypeSystem().getMaxPrecision(SqlTypeName.TIMESTAMP);
      if (expr.precision() > maxPrecision) {
        throw new UnsupportedOperationException(
            String.format(
    ...
    @Override
    public RelDataType visit(final Type.PrecisionTimestampTZ expr) throws RuntimeException {
    ...
    @Override
    public RelDataType visit(final Type.IntervalYear expr) {
      return typeFactory.createTypeWithNullability(
          typeFactory.createSqlIntervalType(YEAR_MONTH_INTERVAL), n(expr));
    }

    @Override
    public RelDataType visit(final Type.IntervalDay expr) {
      return typeFactory.createTypeWithNullability(
          typeFactory.createSqlIntervalType(DAY_SECOND_INTERVAL), n(expr));
    }

    @Override
    public RelDataType visit(final Type.FixedChar expr) {
      return t(n(expr), SqlTypeName.CHAR, expr.length());
    }

    @Override
    public RelDataType visit(final Type.VarChar expr) {
      return t(n(expr), SqlTypeName.VARCHAR, expr.length());
    }

    @Override
    public RelDataType visit(final Type.FixedBinary expr) {
      return t(n(expr), SqlTypeName.BINARY, expr.length());
    }

    @Override
    public RelDataType visit(final Type.Decimal expr) {
      return t(n(expr), SqlTypeName.DECIMAL, expr.precision(), expr.scale());
    }

    @Override
    public RelDataType visit(final Type.Struct expr) {
      if (withinStruct) {
      ...
      try {
        ...
        final List<String> localFieldNames = new ArrayList<>();
        for (final TypeExpression field : expr.fields()) {
          localFieldNames.add(
              fieldNames == null ? "f" + fieldNamePosition : fieldNames.get(fieldNamePosition));
          fieldNamePosition++;
          final ToRelDataType childVisitor =
    ...
    @Override
    public RelDataType visit(final Type.ListType expr) {
      return n(expr, typeFactory.createArrayType(expr.elementType().accept(this), -1));
    }

    @Override
    public RelDataType visit(final Type.Map expr) {
      return n(expr, typeFactory.createMapType(expr.key().accept(this), expr.value().accept(this)));
    }

    @Override
    public RelDataType visit(final Type.UserDefined expr) throws RuntimeException {
      final RelDataType type = userTypeMapper.toCalcite(expr);
      if (type != null) {
        return type;
      }
      throw new UnsupportedOperationException(
    ...
    private boolean n(final NullableType type) {
      return type.nullable();
    }

    private RelDataType t(
        final boolean nullable, final SqlTypeName typeName, final Integer... props) {
    ...
    private RelDataType n(final Type substraitType, final RelDataType type) {
